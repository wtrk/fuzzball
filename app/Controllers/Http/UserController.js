...
class UserController {
  async entireDb({ request, response }) {
    ...
    let finalData=newArraySortedByUID.forEach((e,i)=>{
      if(e.status===2){
        newArraySortedByUID.map((eSub,i)=>{
          if(e.UID===eSub.UID) eSub.status_filtering=2
        })
      }
    })
    axios({
  ...
  async insertEntireInDB({request,response}){
    ...
    for (var key in dataToSave) {
          await Database.table("uid").where("id", dataToSave[key].id).update({
            ...
            status_filtering:dataToSave[key].status_filtering
          });
        ...
        if (entryData.YOB == eParent.YOB && 
          ...
          fuzzball.token_set_ratio(parentString, childString) > fuzz_score
            ...
            let status = 2;
              ...
              if(flag_email===1 && flag_mobile===1 && flag_marital===1 && flag_master===1){
                status = 1
              }
              dataToReturn = {
                fuzz_score,
